import {AudioWaveform, File as FileIcon, FileImage, FolderArchive, Video} from "lucide-react";
...
export const generateThumbnails = (filesToUpload: File[]) => {
  ...
  filesToUpload.forEach((f) => {
    if (f.type.startsWith("image")) {
      newThumbnails[f.name] = URL.createObjectURL(f as unknown as Blob);
    }
  });
...
export const getFileUrl = (file: FileUploadProgress) => {
  return `/api/view?fileName=${file.newFileName}`;
};