...
export function CollapseMenuButton({
  ...
  return isOpen === true || isOpen === Openings.True ? (
    <Collapsible
      open={isCollapsed}
      ...
      >
        ...
        >
          <div className="w-full items-center flex justify-between">
            ...
            <div
              className={cn(
                "whitespace-nowrap p-1 rounded-md border border-muted",
                isOpen
                  ? "translate-x-0 opacity-100"
                  : "-translate-x-96 opacity-0"
              )}
      ...
      <CollapsibleContent className="overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down">
        {submenus.map(({ href, label, active }, index) => (
          <Button
            key={index}
            variant={active ? "secondary" : "ghost"}
            className="w-full justify-start h-10 px-2 gap-1.5 rounded-sm"
            asChild
          >
            <Link href={`${href}?${searchParams.toString()}`}>
              <span className="ml-2">
                <Dot size={36} className="" />
              </span>
              <p
                className={cn(
